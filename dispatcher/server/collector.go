...
type taskResultCollector struct {
	...
	msgCh        chan *pb.SendResultRequest
...
func (a *taskResultCollector) writeToRedis(lst []*pb.SendResultRequest) {
	fmt.Println("finish numbers", len(lst))
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
	defer cancel()
	pipeline := a.rdb.Pipeline()
	for _, msg := range lst {
		...
		// 	pipeline.HSetNX(ctx, SessionTaskResponse(r.SessionId, r.ClientId), r.TaskId, msg.Body)
		// } else {
...
func NewTaskResultCollector(sid string) *taskResultCollector {
	...
	a := &taskResultCollector{
		sessionId:    sid,
		msgCh:        make(chan *pb.SendResultRequest, batchSize*2),
		Topic:        sid + "." + "Collector",
